...
const WorkoutReducer: Reducer<WorkoutsState> = (state = initial, { type, payload = null }) => {
  ...
  switch (type) {
    ...
    case WORKOUT.SET_START:
      return {
        ...state,
        emittedChangeEvent: true, current: { ...current, datetime: { ...datetime, start: payload } }
      }
    case WORKOUT.SET_END:
      return {
        ...state, emittedChangeEvent: true, current: { ...current, datetime: { ...datetime, end: payload } }
      }
    ...
    case WORKOUT.NEW_SEQUENCE:
      return {
        ...state, current: { ...current, sequenceList: [...sequenceList, []], }, emittedChangeEvent: true, exercise: null
      }
    case WORKOUT.DUPLICATE_ITEM:
      ...
      return {
        ...state, emittedChangeEvent: true, cursor: [sequenceIndex, sequence.length - 1], current: { ...current, sequenceList: newSequenceList }
      }
    case WORKOUT.DELETE_ITEM:
      ...
      newSequenceList[sequenceIndex] = sequence;
      return { ...state, emittedChangeEvent: true, current: { ...current, sequenceList: newSequenceList } };
    case WORKOUT.UPDATE_SEQUENCE:
      ...
      return { ...state, current: { ...current, sequenceList: outerList }, cursor: undefined, emittedChangeEvent: true, exercise: null };
    ...
    case WORKOUT.TRIGGER_CHANGE:
      ...
      return { ...state, hasChanges: true, emittedChangeEvent: false }
