...
const ShowWorkout = ({ workout, editable, addNewSequence }: ShowWorkoutProps) => {
  ...
  return (
    <Segment className='workout display' basic>
      ...
      <Segment basic>
        {sequenceList.length ? sequenceList.map((sequence, s) => <ShowSequence compact={false} key={uuid()} index={s} editable sequence={sequence} />) : <List.Item content="No activity recorded" />}
      </Segment>
